...
#define OUTLIER_MIN_COUNT 1520
#endif

ULLI memoryTracker=0;
int showInterval=0;
int numThreadsOpen=0;
...
pthread_mutex_t threadStart = PTHREAD_MUTEX_INITIALIZER;
vector<int> threadsStarted;
...
bool waitForThreadToStartCorrectly(int whichOne);
...
public:
	...
	void evaluate() {
		...
		//bool once=true;
		char temp[500];
		ChangeDir("/home/gvuser/stage2ZerosRemoved");
		patients.clear();
		...
		for(auto t:patients) {
			...
			//cout << hostname << " doing file: " << p << endl;
		}
		...
		for(int j=0;j<totalTestSize;++j) {
			...
			cout << fixed << NNlayersQ[outputsIndex].atNeuronOutputs[0] << endl;
			float out=NNlayersQ[outputsIndex].atNeuronOutputs[0];
			sprintf(temp,"%s,%.10f",testFiles[j].c_str(),out);
			submission.push_back(string(temp));
		}
		fstream ofile("/home/gvuser/MPIversion/submissionFile.csv",ios::out|ios::binary);
		if(ofile.is_open()) {
	...
	void train_kaggle(ULLI maxIter, float lRate, bool vlRate, float RMSwant, int totalTestSize, int totalTrainSize) {
		...
		for(;!threadExit && epoch<maxIter;) {// && (maxTestRight!=totalTestSize && maxTrainRight!=totalTrainSize);) {//} && minRMS>RMSwant;) {
			...
			while(showIntervalCountDown) {
				for(int j=0;j<trainSetSize;++j) {
					...
					//doPred=false;
					//pred=(int)setLabeledPatients[toTrainList[j]].second;
					if(!doingMNIST && !my_rank) {//} && pred) {
						tempPred=0;
						if(NNlayersQ[outputsIndex].atNeuronOutputs[0]>0.5f) {
							tempPred=1;
						}
						tempPred2=0;
						if(setLabeledPatients[toTrainList[j]].second>0.5f) {
							tempPred2=1;
						}
						cout << hostname+" backprop file: "+setLabeledPatients[toTrainList[j]].first << " should be " << tempPred2 << " pred: " << tempPred << " is: " << NNlayersQ[outputsIndex].atNeuronOutputs[0] << "\r";//endl;
						/*temp=hostname+" forward feed processing file: "+setLabeledPatients[toTrainList[j]].first+" prediction is ";
						float yhat=NNlayersQ[outputsIndex].atNeuronOutputs[0]+0.5f;
						float yi=setLabeledPatients[toTrainList[j]].second;
						int yii=(int)yi;
						int yhatt=(int)yhat;
						if(yii==yhatt) {
							temp+="right";
						} else {
							temp+="wrong "+to_string(setLabeledPatients[toTrainList[j]].second)+" yhat: "+to_string(NNlayersQ[outputsIndex].atNeuronOutputs[0]);
							//cout << temp << endl;
						}*/
						//cout << temp;
					}
					...
					for(int i=0;i<outputsIndex;++i) {
						...
						//__gnu_parallel::for_each(NNlayersQ[i].counterW.begin(),NNlayersQ[i].counterW.end(),update_w(&NNlayersQ[i].weightsMatrix[0],&NNlayersQ[i].innerDeltaW[0],lRate));
						//__gnu_parallel::for_each(NNlayersQ[ii].counterN.begin(),NNlayersQ[ii].counterN.end(),update_b(&NNlayersQ[ii].biases[0],&NNlayersQ[ii].innerDeltaB[0],lRate));
						__gnu_parallel::transform(NNlayersQ[ii].innerDeltaB.begin(),NNlayersQ[ii].innerDeltaB.end(),NNlayersQ[ii].outerDeltaB.begin(),NNlayersQ[ii].outerDeltaB.begin(),plus<float>());
						__gnu_parallel::transform(NNlayersQ[i].innerDeltaW.begin(),NNlayersQ[i].innerDeltaW.end(),NNlayersQ[i].outerDeltaW.begin(),NNlayersQ[i].outerDeltaW.begin(),plus<float>());//*/
					}
				...
				/*for(int i=0;i<outputsIndex;++i) {
					ii=i+1;
					__gnu_parallel::transform(NNlayersQ[i].outerDeltaW.begin(), NNlayersQ[i].outerDeltaW.end(), NNlayersQ[i].outerDeltaW.begin(), bind1st(multiplies<float>(),toDivide));
					...
					__gnu_parallel::transform(NNlayersQ[ii].outerDeltaB.begin(), NNlayersQ[ii].outerDeltaB.end(), NNlayersQ[ii].outerDeltaB.begin(), bind1st(multiplies<float>(),toDivide));
			...
			//}
			//if(!doingMNIST) {
			//	cout << " -- " << hostname << " done with work.  Waiting for network...\n";
			//}
			...
			for(int i=0;i<outputsIndex;++i) {
				...
				/*__gnu_parallel::transform(NNlayersQ[i].weightsMatrix.begin(),NNlayersQ[i].weightsMatrix.end(),NNlayersQ[i].weightsMatrix.begin(),divThreads((float)trainSetSize*(float)showInterval));
				__gnu_parallel::transform(NNlayersQ[ii].biases.begin(),NNlayersQ[ii].biases.end(),NNlayersQ[ii].biases.begin(),divThreads((float)trainSetSize*(float)showInterval));//*/
				/*__gnu_parallel::transform(NNlayersQ[i].outerDeltaW.begin(),NNlayersQ[i].outerDeltaW.end(),NNlayersQ[i].outerDeltaW.begin(),divThreads((float)trainSetSize*(float)showInterval));
				__gnu_parallel::transform(NNlayersQ[ii].outerDeltaB.begin(),NNlayersQ[ii].outerDeltaB.end(),NNlayersQ[ii].outerDeltaB.begin(),divThreads((float)trainSetSize*(float)showInterval));//*/
				...
				//NNlayersQ[i].weightsMatrix=NNlayersQ[i].innerDeltaW;
				//NNlayersQ[ii].biases=NNlayersQ[ii].innerDeltaB;
				...
				//__gnu_parallel::transform(NNlayersQ[i].innerDeltaW.begin(),NNlayersQ[i].innerDeltaW.end(),NNlayersQ[i].weightsMatrix.begin(),multi_helper((float)totalTrainSize));
				//__gnu_parallel::transform(NNlayersQ[ii].innerDeltaB.begin(),NNlayersQ[ii].innerDeltaB.end(),NNlayersQ[ii].biases.begin(),multi_helper((float)totalTrainSize));//*/

				/*__gnu_parallel::transform(NNlayersQ[i].outerDeltaW.begin(),NNlayersQ[i].outerDeltaW.end(),NNlayersQ[i].outerDeltaW.begin(),multi_helper((float)trainSetSize));
				__gnu_parallel::transform(NNlayersQ[ii].outerDeltaB.begin(),NNlayersQ[ii].outerDeltaB.end(),NNlayersQ[ii].outerDeltaB.begin(),multi_helper((float)trainSetSize));//*/
				/*__gnu_parallel::transform(NNlayersQ[i].outerDeltaW.begin(),NNlayersQ[i].outerDeltaW.end(),NNlayersQ[i].outerDeltaW.begin(),divThreads((float)trainSetSize*(float)showInterval));
				__gnu_parallel::transform(NNlayersQ[ii].outerDeltaB.begin(),NNlayersQ[ii].outerDeltaB.end(),NNlayersQ[ii].outerDeltaB.begin(),divThreads((float)trainSetSize*(float)showInterval));//*/
				/*__gnu_parallel::transform(NNlayersQ[i].outerDeltaW.begin(),NNlayersQ[i].outerDeltaW.end(),NNlayersQ[i].outerDeltaW.begin(),divThreads((float)trainSetSize));
				__gnu_parallel::transform(NNlayersQ[ii].outerDeltaB.begin(),NNlayersQ[ii].outerDeltaB.end(),NNlayersQ[ii].outerDeltaB.begin(),divThreads((float)trainSetSize));//*/
				MPI_Barrier(MPI_COMM_WORLD);
				...
				/*__gnu_parallel::transform(NNlayersQ[i].innerDeltaW.begin(),NNlayersQ[i].innerDeltaW.end(),NNlayersQ[i].innerDeltaW.begin(),divThreads((float)totalTrainSize));
				__gnu_parallel::transform(NNlayersQ[ii].innerDeltaB.begin(),NNlayersQ[ii].innerDeltaB.end(),NNlayersQ[ii].innerDeltaB.begin(),divThreads((float)totalTrainSize));//*/
				/*__gnu_parallel::transform(NNlayersQ[i].innerDeltaW.begin(),NNlayersQ[i].innerDeltaW.end(),NNlayersQ[i].innerDeltaW.begin(),divThreads((float)num_nodes));
				__gnu_parallel::transform(NNlayersQ[ii].innerDeltaB.begin(),NNlayersQ[ii].innerDeltaB.end(),NNlayersQ[ii].innerDeltaB.begin(),divThreads((float)num_nodes));//*/

				__gnu_parallel::transform(NNlayersQ[i].innerDeltaW.begin(), NNlayersQ[i].innerDeltaW.end(), NNlayersQ[i].innerDeltaW.begin(), multi_helper(lRate/(float)totalTrainSize));//(float)showInterval)));
				//__gnu_parallel::transform(NNlayersQ[i].innerDeltaW.begin(), NNlayersQ[i].innerDeltaW.end(), NNlayersQ[i].innerDeltaW.begin(), multi_helper(lRate));//(float)showInterval)));
				__gnu_parallel::transform(NNlayersQ[i].weightsMatrix.begin(), NNlayersQ[i].weightsMatrix.end(), NNlayersQ[i].innerDeltaW.begin(), NNlayersQ[i].weightsMatrix.begin(), minus<float>());
				__gnu_parallel::transform(NNlayersQ[ii].innerDeltaB.begin(), NNlayersQ[ii].innerDeltaB.end(), NNlayersQ[ii].innerDeltaB.begin(), multi_helper(lRate/(float)totalTrainSize));//*(float)showInterval)));
				//__gnu_parallel::transform(NNlayersQ[ii].innerDeltaB.begin(), NNlayersQ[ii].innerDeltaB.end(), NNlayersQ[ii].innerDeltaB.begin(), multi_helper(lRate));//*(float)showInterval)));
				__gnu_parallel::transform(NNlayersQ[ii].biases.begin(), NNlayersQ[ii].biases.end(), NNlayersQ[ii].innerDeltaB.begin(), NNlayersQ[ii].biases.begin(), minus<float>());//*/
			}
			...
			for(int j=0;j<trainSetSize;++j) {
				...
				if(!printWrongOnes) {
					if(!my_rank && !doingMNIST) {
						...
						cout << hostname+" train set file: "+setLabeledPatients[toTrainList[j]].first << " should be " << tempPred2 << " pred: " << tempPred << " is: " << NNlayersQ[outputsIndex].atNeuronOutputs[0] << "\r";
					}
				} else {
					if(!doingMNIST) {
						tempPred=0;
						...
						if(tempPred2!=tempPred) {
							cout << hostname+" train set file: "+setLabeledPatients[toTrainList[j]].first << " should be " << tempPred2 << " pred: " << tempPred << " is: " << NNlayersQ[outputsIndex].atNeuronOutputs[0] << "\n";
						}
					}
				}
			...
			for(int j=0;j<testSetSize;++j) {
				...
				if(!printWrongOnes) {
					if(!my_rank && !doingMNIST) {
						...
						tempPred2=0;
						if(setLabeledPatients[toTestList[j]].second>0.5f) {
							tempPred2=1;
						}
						cout << hostname+" test set file: "+setLabeledPatients[toTestList[j]].first << " should be " << tempPred2 << " pred: " << tempPred << " is: " << NNlayersQ[outputsIndex].atNeuronOutputs[0] << "\r";
					}
				} else {
					if(!doingMNIST) {
						tempPred=0;
						...
						if(tempPred2!=tempPred) {
							cout << hostname+" test set file: "+setLabeledPatients[toTestList[j]].first << " should be " << tempPred2 << " pred: " << tempPred << " is: " << NNlayersQ[outputsIndex].atNeuronOutputs[0] << "\n";
						}
					}
				}
...
int main(int argc, char *argv[]) {
	...
	threadsStarted=vector<int>(numberOfProcessors,0);
    ...
    for(int i=1;i<argc;++i) {
        ...
        if(temp.find("stage2")!=string::npos) {
        	doStage2=true;
        }
    ...
    if(doPreproc) {
    	//1286126 inputs is the magic number
    	if(doStage2) {
    		ChangeDir("..");
    		ChangeDir("stage2");
    	} else {
				...
				while(whichPatient<toDo) {
					for(int i=0;i<numberOfProcessors && whichPatient<toDo;++i) {
						...
						if(threadsDone[i]) {
		        			...
		        			(*arg).my_id=i;
		        			threadsDone[i]=false;
		        			pthread_mutex_lock(&threadStart);
		        			threadsStarted[i]=0;
		        			pthread_mutex_unlock(&threadStart);
		        			if(staging==REMOVEZEROS) {
		        				err=pthread_create(&threads[i], &attr, preprocThread2, arg);
		        				if(err!=0) {
		        					cout << hostname << " thread creation failed: " << i <<" error code: " << err << " numThreadsOpen: " << numThreadsOpen << endl;
		        				} else {
		        					threadStartFine=waitForThreadToStartCorrectly(i);
		        				}
		        			}
		        			if(staging==GETMINMAX) {
		        				err=pthread_create(&threads[i], &attr, preprocMaxMinThread, arg);
		        				if(err!=0) {
		        					cout << hostname << " thread creation failed: " << i <<" error code: " << err << " numThreadsOpen: " << numThreadsOpen << endl;
		        				} else {
		        					threadStartFine=waitForThreadToStartCorrectly(i);
		        			...
		        			if(staging==DOPREPROC){
		        				err=pthread_create(&threads[i], &attr, preprocThread, arg);
		        				if(err!=0) {
		        					cout << hostname << " thread creation failed: " << i <<" error code: " << err << " numThreadsOpen: " << numThreadsOpen << endl;
		        				} else {
		        					threadStartFine=waitForThreadToStartCorrectly(i);
			...
			while(staging) {
		    	...
		    	if(staging==DOPREPROC) {
		    		...
		    		if(doStage2) {
						...
						string prefix="/home/gvuser/MPIversion/KagglePre/";
						//if(!doStage2) {
							ChangeDir("..");
						//} else {
						//	prefix="/home/gvuser/stage2Preprocessed/";
						//	ChangeDir("..");
						//	ChangeDir("MPIversion");
						//}
						ULLI ffsize;

						ifstream iFile(prefix+patients2[0],ios::in | ios::binary | ios::ate);
						if (iFile.is_open()) {
							ffsize = iFile.tellg();
						  	iFile.close();
						} else {
	...
	ifstream solFile("/home/gvuser/MPIversion/stage1_solution.csv",ios::in|ios::binary);
	once=false;
	int indexx=setLabeledPatients.size();
	int counter=0;
	if(solFile.is_open()) {
		testSet.clear();
		testLabels.clear();
		while(solFile.getline(temp,500)) {
			if(!once) {
				once=true;
			} else {
				vector<string> splitTemp=split(string(temp),",");
				//testSet.push_back(pair<string, int>(splitTemp[0],indexx));
				//testLabels.push_back(pair<float, int>((float)(splitTemp[1][0]-48),indexx));
				trainingSet.push_back(pair<string, int>(splitTemp[0],indexx));
				trainingLabels.push_back(pair<float, int>((float)(splitTemp[1][0]-48),indexx));
				setLabeledPatients.push_back(pair<string,float>(splitTemp[0],(float)(splitTemp[1][0]-48)));
				totalSize=setLabeledPatients.size();
				++indexx;
			}
		}
		solFile.close();
	} else {
		cout << "Couldn't find stage1_solution file\n";
		exit(1);
	}
	if(!doingMNIST) {
		ChangeDir("..");
	...
	//int trainingSize=totalSize-testSet.size();
	int trainingSize=totalSize;
	...
	if(eval) {
		if(!my_rank) {
			...
			ChangeDir("..");
			string fnamee="/home/gvuser/stage2ZerosRemoved/"+patients[my_rank];
	    	ifstream inFileA(fnamee,ios::in | ios::binary | ios::ate);
		    if (inFileA.is_open()) {
			   	fsize = inFileA.tellg();
			   	inFileA.close();
			} else {
				cout << "couldn't find data: " << fnamee << "\n";
				MPI_Finalize();
				exit(1);
			}
...
bool waitForThreadToStartCorrectly(int whichOne) {
	...
	while(threadCheck==0) {
		//pthread_mutex_lock(&threadStart);
		threadCheck=threadsStarted[whichOne];
		//pthread_mutex_unlock(&threadStart);
	}
	if(threadCheck==1) {
		++numThreadsOpen;
...
void getFile(string filename) {
	if(doingMNIST) {
		...
		if(!eval) {
			filename="/home/gvuser/KagglePre2/"+filename+".dat";
		} else {
			filename="/home/gvuser/stage2ZerosRemoved/"+filename+".dat";
		}
		ifstream inFile(filename,ios::in | ios::binary);
	   	if (inFile.is_open()) {
...
int getFile(string filename, UNCHAR *input) {
	if(doStage2) {
		filename="/home/gvuser/stage2Preprocessed/"+filename;
	} else {
	...
	inFile3.seekg(0, inFile3.beg);
   	if (inFile3.is_open()) {
   		...
   		cout << "GetFile2 couldn't find file: " << filename << "\n";
   		return size;
   	}
   	return size;
}
...
void putZeroedFile(string filename, UNCHAR *input, int newSize) {
	if(doStage2) {
		filename="/home/gvuser/stage2ZerosRemoved/"+filename;
	} else {
...
void *preprocThread2(void *thread_parm) {
	...
	putZeroedFile(filename2, &newFile[0], newFile.size());
...
void *preprocThread(void *thread_parm) {
	...
	if (test<0) {
		...
		threadsDone[my_id]=true;
		pthread_mutex_unlock(&crmutex);
		cout << hostname << " thread: " << my_id << " failed\n";
		exit(1);
		pthread_exit(0);
		good=false;
	} 
